...
export default defineConfig(({ mode }) => {
  ...
  return {
    define: {
      ...
      // Inject the Key with the provided fallback to ensure it works out of the box
      'process.env.VITE_SUPABASE_ANON_KEY': JSON.stringify(env.VITE_SUPABASE_ANON_KEY || env.SUPABASE_ANON_KEY || 'sb_publishable_AqTmInNQoIEGNjleHCbAEQ_fQZEFK3s'),
    },
  };
});